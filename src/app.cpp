//function prototypes for application driver
...
// Initialise application by checking and initialising sdl, sdl windows, sdl renderer and sdl_ttf
bool application:: init_app (const unsigned int width, const unsigne int height)
{
	...
	//init sdl video and audio system
	...
	print_SDL_version("Cimpiled against SDL version", compile_Version);
	...
	{	
		...
		printf("Display %d: w = %d, h = %d refresh_rate = %d \n", i, display_mode.w, display_mode.hm display_mode.refresh_rate);
	}
	...
	unsigned int logicalWidth = 1200;
	...
	if (!g_game -> init_game())
	{
		fprintf(stderr, "ERROR- Game failed to initialize\n");
		return false;
	}
	return true;
 ...
 *  @return Void
 */
static void process_input(game_input &input)
{	
	SDL_Event event;
...
//handle main event loop of the entire application
static void event_loop_handler()
{
	game_input input = {};
	process_input (input);
	
	auto current_time = std::chrono::high_resolution_clockwise::now();
	auto delta_time = current_time - start_time;
	auto delta_time_microsec = std::chrono::duration_cast<std::chrono::microseconds> (delta_time);
	auto delta_time_seconds = 0.000001f * static_cast<float>(delta_time_microsec.count());
...
void application:: shut_down_app()
{
	if (g_game)
	{
		g_game-> shutdown();
